...
def readCameraMovie(path, transformsfile, white_background, extension=".png"):
    '''
    Return a camera object containing the poses only to prep for movie
    '''
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            image_path = ""
            image_name = "black_image"
            width = 1080
            height = 1080
            arr = np.random.rand(width, height)
            image = Image.fromarray(arr*255.0, "RGB")
...
def readNerfSyntheticInfoMovie(path, white_background, eval, extension=".png"):
    '''
    ...
    scene_info = SceneInfo(point_cloud=pcd,
                           train_cameras=movie_cam_infos,
                           test_cameras=movie_cam_infos,
                           nerf_normalization=nerf_normalization,
                           ply_path=ply_path)
    return scene_info
...
sceneLoadTypeCallbacks = {
    ...
    "Blender_movie": readNerfSyntheticInfoMovie
